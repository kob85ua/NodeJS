...
module.exports = class ContactController {
  ...
  static async findContactBySubscription(req, res) {
    const { sub } = req.query;

    const contacts = await Contact.find({ subscription: sub });
    if (contacts.length === 0) {
      return res.status(404).send('Contacts are not found');
    }
  ...
  static validateQuery(req, res, next) {
    const { sub } = req.query;
    const subQuery = Joi.string().valid('free', 'pro', 'premium');
    const result = subQuery.validate(sub);
    if (result.error) {
      return res.status(400).json({ message: result.error.details[0].message });
    }
